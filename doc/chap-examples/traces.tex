...
\cmdline{\$PATH/traces/pin}.  On Linux, running the ./getpin.sh script
from the \cmdline{\$PATH/traces/pintraces} directory will
...
Also download PIN and extract it to the \cmdline{\$PATH/traces/pin}
...
the \cmdline{\$PATH/traces/libtracewrap/libtrace/protobuf/vsprojects}
...
prompt, navigate to
\cmdline{\$PATH/traces/libtracewrap/libtrace/src/cpp} and run
\cmdline{make -f Makefile.windows} to build protobuffers and the
libtrace library.

The PIN tool itself can be built by executing \cmdline{make} in the
\cmdline{\$PATH/traces/pintraces} directory on both Windows and Linux.
