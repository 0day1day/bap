...
\cmdline{\$BAPPATH/bap}, then PIN should be in
\cmdline{\$BAPPATH/bap/pin}.  On Linux, running the ./getpin.sh script
from the \cmdline{\$BAPPATH/bap/pintraces} directory will
...
as well.  First, make sure that \cmdline{\$BAPPATH} contains no
...
\cmdline{\$BAPPATH/bap/pin} directory.  Next, we need to upgrade the
...
navigate to the
\cmdline{\$BAPPATH/bap/libtracewrap/libtrace/protobuf/vsprojects}
...
prompt, navigate to \cmdline{\$BAPPATH/bap/libtracewrap/libtrace/src/cpp}
...
\cmdline{\$BAPPATH/bap/pintraces} directory on both Windows and Linux.
After compilation, the PIN tool should exist in
\\ \cmdline{\$BAPPATH/bap/pintraces/obj-ia32/gentrace.so} (or
