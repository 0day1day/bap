...
module RECURSIVE_DESCENT_SPEC = struct
  ...
  let get_succs g (v,l,e) =
    ...
    | Jmp (e, _)::_, _ ->
      ...
      | Some l -> Addrs [l]
      | None -> Indirect)
    | CJmp (e, te, fe, _)::_, Some ((true|false) as b, _) ->
      (match lab_of_exp (if b then te else fe) with
      | Some l -> Addrs [l]
      | None -> Indirect)
    | CJmp _::_, _ -> failwith "error"
    | _::_, _ -> failwith "We need successor information here"
    | [], _ -> Addrs []
...
module Make(D:DISASM) = struct
  let disasm_at p addr =
    ...
    let add_resolved_edge (s,l,e) c lbl  =
      ...
      | Addr a ->
        (try c, CA.find_label c lbl
         with Not_found ->
           let (prog, next) = Asmir.asm_addr_to_bap p a in
           let (c', edges, bbs) = Cfg_ast.add_prog c prog in
            (* Queue edges *)
