...
(* FIXME: Handle subregs! *)
  ...
  let op2str = function
    ...
    | Retn (op, _) -> 
      ...
      | None -> "ret")
    | Nop -> "nop"
    | Mov(t,d,s,None) -> Printf.sprintf "mov %s, %s" (opr ~t d) (opr ~t s)
    | Mov(t,d,s,Some(_)) -> Printf.sprintf "cmov %s, %s" (opr ~t d) (opr ~t s)
    | Movs(t) -> "movs"
    ...
    | Movdq(_t,td,d,ts,s,vs,align,name) ->
      (match vs with 
    ...
    | Rotate (rt, t, src, shift, use_cf) -> 
      let base = match rt with
        ...
        | _ -> disfailwith "impossible" in
      Printf.sprintf "%s %s, %s" base (opr ~t src) (opr shift)
    | Hlt -> "hlt"
    | Rdtsc -> "rdtsc"
    | Cpuid -> "cpuid"
    ...
    | Inc (t, o) -> Printf.sprintf "inc %s" (opr ~t o)
    | Dec (t, o) -> Printf.sprintf "dec %s" (opr ~t o)
    | Jump a -> Printf.sprintf "jmp %s" (j2str a)
    | Bt(t,d,s) -> Printf.sprintf "bt %s, %s" (opr ~t d) (opr ~t s)
    | Bs(t,d,s,dir) -> Printf.sprintf "bs%s %s, %s" (opr ~t d) (opr ~t s) (match dir with Forward -> "f" | Backward -> "r")
    | Jcc _ -> "jcc"
    | Setcc _ -> "setcc"
    | Cmps _ -> "cmps"
    | Scas _ -> "scas"
    | Stos _ -> "stos"
    | Push(t,o) -> Printf.sprintf "push %s" (opr ~t o)
    | Pop(t,o) -> Printf.sprintf "pop %s" (opr ~t o)
    | Pushf _ -> "pushf"
    | Popf _ -> "popf"
    | Sahf -> "sahf"
    | Lahf -> "lahf"
    | Add(t,d,s) -> Printf.sprintf "add %s, %s" (opr ~t d) (opr ~t s)
    | Adc(t,d,s) -> Printf.sprintf "adc %s, %s" (opr ~t d) (opr ~t s)
    | Sub(t,d,s) -> Printf.sprintf "sub %s, %s" (opr ~t d) (opr ~t s)
    | Sbb(t,d,s) -> Printf.sprintf "sbb %s, %s" (opr ~t d) (opr ~t s)
    | Cmp(t,d,s) -> Printf.sprintf "cmp %s, %s" (opr ~t d) (opr ~t s)
    | Cmpxchg(t,d,s) -> Printf.sprintf "cmpxchg %s, %s" (opr ~t d) (opr ~t s)
    | Cmpxchg8b(o) -> Printf.sprintf "cmpxchg8b %s" (opr o)
    | Xadd(t,d,s) -> Printf.sprintf "xadd %s, %s" (opr ~t d) (opr ~t s)
    | Xchg(t,d,s) -> Printf.sprintf "xchg %s, %s" (opr ~t d) (opr ~t s)
    | And(t,d,s) -> Printf.sprintf "and %s, %s" (opr ~t d) (opr ~t s)
    | Or(t,d,s) -> Printf.sprintf "or %s, %s" (opr ~t d) (opr ~t s)
    | Xor(t,d,s) -> Printf.sprintf "xor %s, %s" (opr ~t d) (opr ~t s)
    | Test(t,d,s) -> Printf.sprintf "test %s, %s" (opr ~t d) (opr ~t s)
    | Ptest(t,d,s) -> Printf.sprintf "ptest %s, %s" (opr ~t d) (opr ~t s)
    | Not(t,o) -> Printf.sprintf "not %s" (opr ~t o)
    | Neg(t,o) -> Printf.sprintf "neg %s" (opr ~t o)
    | Mul (t, src) -> 
      Printf.sprintf "mul %s" (opr ~t src)
    | Imul (t, (b,dst), src1, src2) -> 
      ...
      | true ->
        Printf.sprintf 
          "imul %s"  (opr ~t src2)
      | false ->
        Printf.sprintf "imul %s, %s, %s" (opr ~t dst) (opr ~t src1) (opr ~t src2))
    | Div(t, src) ->
      Printf.sprintf "div %s" (opr ~t src)
    | Idiv(t, src) ->
      Printf.sprintf "idiv %s" (opr ~t src)
    | Cld -> "cld"
    | Leave _ -> "leave"
...
(*  and parse_modrmxmm a =
  ...
  let parse_modrm_addr_vec rex vex at ia a =
  ...
  let get_opcode pref ({rex; vex; rm_extend; addrsize} as prefix) a =
    ...
    in 
    let parse_modrm_vec = parse_modrm_addr_vec rex vex addrsize a in
      ...
      (
        ...
        let (o1, o2, na) = match b1 & 7 with
          | 0 | 1 -> let r, rm, na = parse_modrm_addr na in
                     (rm, r, na)
          | 2 | 3 -> let r, rm, na = parse_modrm_addr na in
                     (r, rm, na)
    ...
    | 0x0f -> (
      ...
      | 0x1f ->
        ...
        let _, _, na = parse_modrm_addr na in
        (Nop, na)
      | 0x12 | 0x13 | 0x28 | 0x29 | 0x6e | 0x7e | 0x6f | 0x7f | 0xd6 ->
        ...
        let t, name, align, tsrc, tdest, r, rm, rv = match b2 with
          | 0x12 when not (rv = None) -> 
            r128, "movlps", false, r64, r128, tovec r, tovec rm, rv
          | 0x12 | 0x13 when prefix.opsize_override ->
            r128, "movlpd", false, r64, r64, tovec r, tovec rm, None
          | 0x12 | 0x13 when not prefix.opsize_override ->
            r128, "movlps", false, r64, r64, tovec r, tovec rm, None
          | 0x28 | 0x29 when prefix.opsize_override ->
            let mvt = match prefix.vex with None -> r128 | _ -> prefix.opsize in
            mvt, "movapd", true, mvt, mvt, tovec r, tovec rm, None
          | 0x28 | 0x29 when not prefix.opsize_override -> 
            let mvt = match prefix.vex with None -> r128 | _ -> prefix.opsize in
            mvt, "movaps", true, mvt, mvt, tovec r, tovec rm, None
          | 0x6f | 0x7f when prefix.repeat -> 
            let mvt = match prefix.vex with None -> r128 | _ -> prefix.opsize in
            mvt, "movdqu", false, mvt, mvt, tovec r, tovec rm, None
