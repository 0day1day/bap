...
let concrete_stream mem_hash concrete_state thread_map arch block return =
  ...
  if return then
    ...
    ignore(run_block arch concrete_state memv thread_map block);
    []
  )

let concrete return arch =
...
struct
  ...
  let generate_formula_setup mem_hash concrete_state thread_map arch block =
    let block =
  ...
  let generate_formula filename solver =
    ...
    (fun arch ->
      ...
      (fun block ->
        ...
        in
        last_state :=
          Some (TraceSymbolic.symbolic_run_blocks state arch block))),
