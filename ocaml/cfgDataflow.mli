...
sig
  (** [worklist_iterate g] returns a worklist algorithm for graph [g]
      ...
      [v]. *)
  val worklist_iterate : ?init:(D.CFG.G.t -> D.L.t) ->
    D.CFG.G.t -> (D.CFG.G.V.t -> D.L.t) * (D.CFG.G.V.t -> D.L.t)

  (** Like [worklist_iterate], except the dataflow is done on the
      statement level. A statement [bb,n] is the [n]th stmt
      (zero-indexed) in [bb]. *)
  val worklist_iterate_stmt : ?init:(D.CFG.G.t -> D.L.t) ->
...
sig
  ...
  *)
  val worklist_iterate_widen : ?init:(D.CFG.G.t -> D.L.t) ->
    ?nmeets:int -> D.CFG.G.t -> (D.CFG.G.V.t -> D.L.t) * (D.CFG.G.V.t -> D.L.t)
end
