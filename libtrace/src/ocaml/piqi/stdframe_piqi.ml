module rec Stdframe_piqi :
             sig
               type operand_info_specific =
                 [
                   | `mem_operand of Stdframe_piqi.mem_operand
                   | `reg_operand of Stdframe_piqi.reg_operand
                 ]
               
               type taint_info =
                 [
                   | `no_taint
                   | `taint_id of Stdframe_piqi.taint_id
                   | `taint_multiple
                 ]
               ...
               type thread_id = Stdframe_piqi.uint64
               
               type address = Stdframe_piqi.uint64
               
               type bit_length = int
               
               type taint_id = Stdframe_piqi.uint64
               
               type exception_number = Stdframe_piqi.uint64
               
               type std_frame = Std_frame.t
               
               type operand_list = Stdframe_piqi.operand_info list
...
and
  Std_frame :
    sig
      type t =
        { mutable address : Stdframe_piqi.address;
          mutable thread_id : Stdframe_piqi.thread_id;
          mutable rawbytes : Stdframe_piqi.binary;
          mutable operand_list : Stdframe_piqi.operand_list
        }
...
and
  Operand_info :
    sig
      type t =
        {
          mutable operand_info_specific : Stdframe_piqi.operand_info_specific;
          mutable bit_length : Stdframe_piqi.bit_length;
          mutable operand_usage : Stdframe_piqi.operand_usage;
          mutable taint_info : Stdframe_piqi.taint_info;
          mutable value : Stdframe_piqi.binary
        }
...
and
  Mem_operand :
    sig type t = { mutable address : Stdframe_piqi.address }
         end =
    Mem_operand
and
  Operand_usage :
  ...
  in
    Piqirun.gen_record code
      [ _operand_info_specific; _bit_length; _operand_usage; _taint_info;
        _value ]
and
  gen__operand_info_specific code (x : Stdframe_piqi.operand_info_specific) =
  Piqirun.gen_record code
    [ (match x with
...
and gen__taint_info code (x : Stdframe_piqi.taint_info) =
  Piqirun.gen_record code
    [ (match x with
