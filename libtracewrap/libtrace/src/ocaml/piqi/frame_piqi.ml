    ...
    sig
      type t =
        { mutable addr : Frame_piqi.address;
          ...
          mutable source_name : string option;
          mutable offset : Frame_piqi.address option
        }
...
and parse_taint_intro x =
  ...
  let (_source_name, x) = Piqirun.parse_optional_field 4 parse_string x in
  let (_offset, x) = Piqirun.parse_optional_field 5 parse_address x
  in
    (Piqirun.check_unparsed_fields x;
     {
...
and gen__taint_intro code x =
  ...
  let _source_name =
    Piqirun.gen_optional_field 4 gen__string x.Taint_intro.source_name in
  let _offset =
    Piqirun.gen_optional_field 5 gen__address x.Taint_intro.offset
  in
    Piqirun.gen_record code
