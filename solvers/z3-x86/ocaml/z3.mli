...
(**
   ...
   - OP_TO_REAL Coercion of integer to real.
   ...
   - OP_CONST_ARRAY The constant array. For example, select(const(v),i) = v holds for every v and i.

   - OP_ARRAY_DEFAULT Default value of arrays. For example default(const(v)) = v.

   - OP_STORE_ITE Non-pointwise array update. 
         It satisfies select(store_ite(a,b,c),i) = if select(a,i) = select(b,i) then select(c,i) else select(b,i)

   - OP_SET_UNION Set union between two Booelan arrays (two arrays whose range type is Boolean).

   - OP_SET_INTERSECT Set intersection between two Boolean arrays.

   - OP_SET_DIFFERENCE Set difference between two Boolean arrays.

   - OP_SET_COMPLEMENT Set complement of a Boolean array.

   - OP_SET_SUBSET Subset predicate between two Boolean arrays.
   ...
   - OP_ULEQ Unsigned bit-vector <=.

   - OP_SLEQ Signed bit-vector  <=.

   - OP_UGEQ Unsigned bit-vector  >=.

   - OP_SGEQ Signed bit-vector  >=.

   - OP_ULT Unsigned bit-vector  <.

   - OP_SLT Signed bit-vector <.

   - OP_UGT Unsigned bit-vector >.

   - OP_SGT Signed bit-vector >.

   - OP_BAND Bit-wise and.

   - OP_BOR Bit-wise or.

   - OP_BNOT Bit-wise not.

   - OP_BXOR Bit-wise xor.

   - OP_BNAND Bit-wise nand.

   - OP_BNOR Bit-wise nor.

   - OP_BXNOR Bit-wise xnor.

   - OP_CONCAT Bit-vector concatenation.
   ...
   - OP_BREDOR Bit-vector reduce or.

   - OP_BREDAND Bit-vector reduce and.
...
(** 
        ...
        There is no converse operation exposed, 
        but you can take the floor of a real by 
...
(**
       ...
       To print shared common subexpressions only once, use the PRINT_LOW_LEVEL
       mode.
