...
let predicate_stp_solve_test str stp_result (g_cfg, m2actx) =
  ...
  let test_vc (name,vc) =
    ...
    let foralls = List.map (Memory2array.coerce_rvar_state m2actx) [] in 
