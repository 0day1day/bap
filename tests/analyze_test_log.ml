	...
	with Not_found -> (
	  try
		...
		if (!verbose) then (
		  ...
		  print_out "Second exp result:";
		  print_out ("Asm = "^asm);
		  print_out ("Context = "^Array.get matches 1);
		  print_out ("Reg = "^Array.get matches 2);
		  print_out ("Trace = "^Array.get matches 3);
		  print_out ("Eval = "^Array.get matches 4);
		)
