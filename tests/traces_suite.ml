...
let concrete_eval_test (ranges, start_addr) = 
  let prog = Parser.program_from_file halted_file in
  let ctx = Symbeval.concretely_execute ~s:start_addr ~loud:false prog in
  let pat = "R_EAX_" in
  ...
  Var.VarHash.iter 
	(fun k v ->
	  ...
	  | var,Symbeval.Symbolic e ->
		...
		then check_int64_answer e 0x2aL 
		else ()
	  | _ -> ()
    ) ctx.Symbeval.delta;;
